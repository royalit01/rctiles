<?php
session_start();
// Make the session cookie secure by using HTTPS only and HTTPOnly flag
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT user_id, first_name, last_name, password, role_id FROM users WHERE phone_no = ?";
                ...
                if (password_verify($password, $hashed_password)) {
                    ...
                    switch ($role_id) {
                        case 1:  // Assuming 1 is the role_id for Admin
                            header("location: ../Main Dashboard/admin_dashboard.php");
                            break;
                        case 2:  // Assuming 2 is the role_id for Manager
                            header("location: ../Main Dashboard/manager_dashboard.php");
                            break;
                        case 3:  // Assuming 3 is the role_id for Salesperson
                            header("location: ../Main Dashboard/sales_dashboard.php");
                            break;
                        case 4:  // Assuming 4 is the role_id for Delivery
                            header("location: ../Main Dashboard/delivery_dashboard.php");
                            break;
                        default:
                            header("location: ../authorization/login.php");  // Default dashboard for other roles
                            break;
                    }   
                    exit;
                } else {
...
<meta charset="utf-8" />
        ...
        <title>Login - RC Ceramic Mall</title>
